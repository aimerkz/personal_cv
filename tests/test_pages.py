...
def test_about_page(mock_st, mock_columns_about, mock_get_hard_skills, mock_display_ed):
    about_page()
...
def test_projects_page(mock_st, mock_title, mock_display_projects):
    project_page()
    assert mock_st.title.return_value == mock_title
    mock_display_projects.assert_called_once()


def test_work_history_page(mock_st, mock_title, mock_display_work_history):
    work_history_page()
    assert mock_st.title.return_value == mock_title
